 ...
 *		    -x: extended redcode: allow spl. [AEB]
...
int	splflag = 1;		/* TRUE --> allow splits */
int	opto;			/* stop when no more own locs */
int	opti;			/* stop when not_own instruction executed */
int	min_spl_dist = 0;
int	max_no_of_procs = 64;	/* Note: not the same as MAXPROCNUM ! */
int	mature_at = 0;		/* Age at which spl is allowed */
...
char **argv;
{
	...
	while (argc > 1 && argv[1][0] == '-' && argv[1][1]) {
		...
		case 'n':	/* num of procs */
			...
			goto skipdigits;
		case 'd':
			if(digit(argv[0][1])) {	/* min spl dist */
		    	    min_spl_dist = atoi(*argv + 1);
			    printf("Min dist to others when splitting: %d\n",
				min_spl_dist);
			    goto skipdigits;
			}
		...
		default:
		 ...
		 printf("\t-x\tdisallow redcode with SPL instruction\n");
		 ...
		 printf("\t-d1\tset min dist to others while splitting to 1\n");
		 exit(1);
		}
	}
...
/*
		if ( strncmp(ip, op_str[SPL], 3) == 0) {
			op = SPL;
			ip += 3;
		} else
*/
		for ( i = 0; i <= SPL; i++)	/* SPL is highest op-code */
		{
		...
		if ( op != JMP && op != SPL) {	/* these have only A operand */
			if ( *ip == '#'){
...
address place;
{
	 ...
	 * here I have chosen for max_no_of_procs and min_spl_dist. Note that
	...
	if(ancestor) {
		...
		for(proc = head_procs; proc; proc = proc->next_proc)
		    if(dist(proc->pc, place) < min_spl_dist)
			return((struct proc *) 0);
	}
